...
class CategoryModel {
  ...
  static async getAllCategories(options = {}) {
    try {
      ...
      // Get categories
      const sql = `
                SELECT id, name, code, slug, description, image_url, is_active, 
  ...
  static async findByCode(code) {
    try {
      const sql = `
                ...
                WHERE code = ?
            `;
      const result = await executeQuery(sql, [code], "Find Category By Code");
      return result.length > 0 ? result[0] : null;
    } catch (error) {
      throw new DatabaseError(
        `Error finding category by code: ${error.message}`,
        error
      );
    }
  }
  ...
  static async createCategory(categoryData) {
    try {
      ...
      if (categoryData.code) {
        const existingCategory = await this.findByCode(categoryData.code);
        if (existingCategory) {
          throw new Error(
  ...
  static async updateCategory(id, updateData) {
    try {
      ...
      if (updateData.code) {
        const existingCategory = await this.findByCode(updateData.code);
  ...
  static async getCategoryWithProductCount(id) {
    try {
      const sql = `
                SELECT c.id, c.name, c.code, c.slug, c.description, c.image_url, 
      ...
      throw new DatabaseError(
        `Error getting category with product count: ${error.message}`,
        error
      );
    }
  }
  ...
  static async getAllCategoriesWithProductCounts(includeEmpty = false) {
    try {
      ...
      const sql = `
                ...
                GROUP BY c.id, c.name, c.code, c.slug, c.description, c.image_url, 
                         c.is_active, c.sort_order, c.created_at, c.updated_at
                ${havingClause}
      ...
      throw new DatabaseError(
        `Error getting categories with product counts: ${error.message}`,
        error
      );
    }
  }
  ...
  static async getCategoryProductCount(categoryId) {
    try {
      ...
      throw new DatabaseError(
        `Error getting category product count: ${error.message}`,
        error
      );
    }
  }
  ...
  static async updateSortOrders(categoryUpdates) {
    try {
      ...
      throw new DatabaseError(
        `Error updating sort orders: ${error.message}`,
        error
      );
    }
  }
  ...
  static async slugExists(slug, excludeId = null) {
    try {
      ...
      throw new DatabaseError(
        `Error checking slug existence: ${error.message}`,
        error
      );
    }
  }
  ...
  static async codeExists(code, excludeId = null) {
    try {
      let sql = "SELECT id FROM categories WHERE code = ?";
      let params = [code];
      ...
      const result = await executeQuery(sql, params, "Check Code Exists");
      return result.length > 0;
    } catch (error) {
      throw new DatabaseError(
        `Error checking code existence: ${error.message}`,
        error
      );
    }
  }
  ...
  static async getCategoryOptions() {
    try {
      ...
      throw new DatabaseError(
        `Error getting category options: ${error.message}`,
        error
      );
    }
  }
