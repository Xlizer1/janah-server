...
const { hash, verifyPassword, createToken, verifyUserToken } = require('../../../helpers/common');
const { 
    ValidationError, 
    AuthenticationError, 
    ConflictError, 
...
class AuthController {
    ...
    static async verifyPhone(req, res) {
        try {
            ...
            res.json({
                status: true,
                message: 'Phone number verified successfully. Your account is pending admin activation.',
                data: {
                    user: updatedUser,
    ...
    static async resendVerificationCode(req, res) {
        try {
            ...
            // Check rate limit
            const rateLimit = await VerificationCodeModel.checkRateLimit(formattedPhone, type);
            ...
            // Send SMS
            await TwilioService.sendVerificationCode(formattedPhone, verificationCode, type);
    ...
    static async login(req, res) {
        try {
            ...
            if (!user.is_active) {
                throw new AuthenticationError('Your account is pending admin activation');
            }
    ...
    static async forgotPassword(req, res) {
        try {
            ...
            // Check rate limit
            const rateLimit = await VerificationCodeModel.checkRateLimit(formattedPhone, 'password_reset');
            if (!rateLimit.canRequest) {
                throw new BusinessLogicError(
            ...
            // Send SMS
            await TwilioService.sendVerificationCode(formattedPhone, resetCode, 'password_reset');
