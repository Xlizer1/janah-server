  ...
  return new Promise(async (resolve, reject) => {
    try {
      bcrypt
        .hash(
          text,
          typeof saltRounds === "string" ? JSON.parse(saltRounds) : saltRounds
        )
...
const createToken = (object) => {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      const token = await jwt.sign({ data: object }, tokenKey, {
...
function getToken(req) {
  return new Promise(async (resolve, reject) => {
    ...
    const token =
      headerAuthorization || req?.headers["jwt"] || req?.headers["token"];
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      ...
      if (data?.data) {
        ...
        console.log("Token data structure invalid:", data);
        resolve(null);
      }
    } catch (error) {
      ...
      console.error(
        "Secret key length:",
        tokenKey ? tokenKey.length : "undefined"
      );
      resolve(null);
    }
  });
