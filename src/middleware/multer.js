...
const { MAX_FILE_SIZE = 52428800 } = process.env; // 50MB default
...
const { uploadMiddlewares, handleMulterError } = require('../utils/fileUpload');
    ...
    if (req.file) {
      res.json({
        ...
        data: {
          ...
          path: req.file.path,
          size: req.file.size,
          url: FileUploadService.getFileUrl(req, req.file.path)
        }
      });
    } else {
      res.status(400).json({
...
router.post('/upload-multiple', 
  uploadMiddlewares.productGallery,
  handleMulterError,
  (req, res) => {
    if (req.files && req.files.length > 0) {
      const files = req.files.map(file => ({
        ...
        path: file.path,
        size: file.size,
        url: FileUploadService.getFileUrl(req, file.path)
      }));
...
const uploadMultipleFiles = async (files) => {
  ...
  const response = await fetch('/api/v1/products/gallery', {
    method: 'POST',
    headers: {
      'Authorization': `Bearer ${token}`
    },
